---
name: share-report
description: 分享报告到 Web：将 MD 报告转 HTML 发布到自己的服务器并返回 URL。Use when 用户要把报告分享成网页链接。
---
...
- 本地项目目录：`projects/<share-site>/`（含 manifest.json、gen_index.py、site/）
...
#### 第 2 步：pandoc 转换（含 SEO + TTS）

```bash
pandoc /tmp/<slug>_no_h1.md \
  -o projects/<share-site>/site/<slug>.html \
  --standalone \
  --embed-resources \
  --metadata title="<报告标题>" \
...
注意：输出直接写到 `projects/<share-site>/site/` 目录。

#### 第 3 步：更新 manifest.json

编辑 `projects/<share-site>/manifest.json`，在 `articles` 数组中添加新条目：
...
```bash
cd projects/<share-site> && python3 gen_index.py
```
...
```bash
rsync projects/<share-site>/site/<slug>.html <your-server>:<your-remote-share-dir>/
ssh <your-server> "chmod 644 <your-remote-share-dir>/<slug>.html"

# 如果更新了 index.html，也上传
rsync projects/<share-site>/site/index.html <your-server>:<your-remote-share-dir>/
ssh <your-server> "chmod 644 <your-remote-share-dir>/index.html"
```

#### 第 6 步：git commit manifest 变更

```bash
cd projects/<share-site> && git add manifest.json && git commit -m "add: <slug>"
```
...
```bash
# 确认图片已内嵌
grep -c 'data:image' projects/<share-site>/site/<slug>.html
# 应输出图片数量（>0）
```
...
- HTML 是自包含的（CSS + 图片 + TTS 均内嵌），不依赖外部文件
- share 目录公开可访问，不要上传敏感内容
- 如需删除已发布内容：`ssh <your-server> "rm <your-remote-share-dir>/<slug>.html"`
...
- 本地项目目录 `projects/<share-site>/` 是 manifest 和 site 的 source of truth
- `site/` 下的 HTML 文件建议在 .gitignore 中忽略，只追踪 manifest.json 和脚本

## 首次使用前的本地准备

由于 skill 中的发布管线依赖一个具体的 `projects/<share-site>/` 项目（含 `manifest.json`、`gen_index.py`、`site/`），首次启用前需要：

1. 在 `projects/` 下建立你自己的 share-site 项目目录
2. 准备好 `manifest.json` 初始结构、`gen_index.py` 索引生成脚本、空的 `site/` 输出目录
3. 把上面所有 `<your-server>` / `<your-remote-share-dir>` / `<share-site>` 占位符在脑子里替换为你的真实值（也可考虑写入个人 .env 或脚本封装）
